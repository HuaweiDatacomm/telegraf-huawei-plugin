...
func (dialin *HuaweiTelemetryDialin) Start(acc telegraf.Accumulator) error {
    dialin.acc = acc
    // init parser
    parseGpb, err := parsers.NewHuaweiGrpcGpbParser()
    parseJson, err := parsers.NewHuaweiGrpcJsonParser()
    if err != nil {
...
func (dialin *HuaweiTelemetryDialin) singleSubscribe(dialinConfig router, parserGpb, parserJson parsers.Parser) {
    ...
    if dialinConfig.Aaa.Username != "" && dialinConfig.Aaa.Password != "" {
        ...
        dec_password, _ := base64.StdEncoding.DecodeString(dialinConfig.Aaa.Password)
